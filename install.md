...
activate cookiecutter_conda_data_science
```
...
To move beyond notebook prototyping, all reusable code should go into the `cookiecutter_conda_data_science/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `cookiecutter_conda_data_science` folder and use the modules inside your notebooks :
...
```python
from cookiecutter_conda_data_science.utils.paths import data_dir
data_dir()
```
